...
?>
                        ...
                        case 'location_meta':
                            ...
                            jQuery(tr).find('.open-mapping-grid-modal').on('click', evt => {
                                ...
                                if (post_location_grid_meta !== undefined && post_location_grid_meta.length !== 0) {
                                    ...
                                    if (gridData.level === 'admin0') {
                                        zoom = 3
                                    } else if (gridData.level === 'admin1') {
                                        zoom = 6
                                    } else if (gridData.level === 'admin2') {
                                        zoom = 10
                                    }
    ...
    public function render_field_as_web_component($field_key, $fields, $post, $show_extra_controls = false, $show_hidden = false, $field_id_prefix = '')
    {
        ...
        ?>
            ...
            ?>
                <dt-single-select class="select-field" <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_attr(key_exists($field_key, $post) ? $post[$field_key]["key"] : null) ?>" options="<?php echo esc_attr(json_encode($this->assoc_to_array($fields[$field_key]["default"]))) ?>">
            ...
            <?php elseif ($field_type === "tags") : ?>
                ...
                ?>
                <dt-tags <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_attr(json_encode($value)) ?>" placeholder="<?php echo esc_html(sprintf(_x("Search %s", "Search 'something'", 'disciple_tools'), $fields[$field_key]['name'])) ?>" allowAdd>
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-tags>
            ...
            <?php elseif ($field_type === "multi_select") : ?>
                ...
                ?>
                <dt-multi-select <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_attr(json_encode($value)) ?>" options="<?php echo esc_attr(json_encode($options)) ?>" placeholder="<?php echo esc_attr(sprintf(_x("Search %s", "Search 'something'", 'disciple_tools'), $fields[$field_key]['name'])) ?>" display="<?php echo esc_attr(isset($fields[$field_key]["display"]) ? $fields[$field_key]["display"] : 'typeahead') ?>">
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-multi-select>
            ...
            <?php elseif ($field_type === "textarea") : ?>
                <dt-textarea <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_html($post[$field_key] ?? "") ?>">
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-textarea>
            <?php elseif ($field_type === "number") : ?>
                <dt-number <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_html($post[$field_key] ?? "") ?>" <?php echo esc_html($disabled); ?> <?php echo isset($fields[$field_key]["min_option"]) && is_numeric($fields[$field_key]["min_option"]) ? 'min="' . esc_html($fields[$field_key]["min_option"] ?? "") . '"' : '' ?> <?php echo isset($fields[$field_key]["max_option"]) && is_numeric($fields[$field_key]["max_option"]) ? 'max="' . esc_html($fields[$field_key]["max_option"] ?? "") . '"' : '' ?>>
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-number>
            <?php elseif ($field_type === "date") : ?>
                <dt-date <?php echo wp_kses_post($shared_attributes) ?> timestamp="<?php echo esc_html($post[$field_key]["timestamp"] ?? '') ?>">
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-date>
            ...
            <?php elseif ($field_type === "connection") : ?>
                ...
                ?>
                <dt-connection <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_attr(json_encode($value)) ?>" data-posttype="<?php echo esc_attr($fields[$field_key]["post_type"]) ?>" allowAdd>
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-connection>
            ...
            <?php elseif ($field_type === "location") : ?>
                ...
                ?>
                <dt-location <?php echo wp_kses_post($shared_attributes) ?> value="<?php echo esc_attr(json_encode($value)) ?>" filters="<?php echo esc_attr(json_encode($filters)) ?>" placeholder="<?php echo esc_html(sprintf(_x("Search %s", "Search 'something'", 'disciple_tools'), $fields[$field_key]['name'])) ?>">
                    <?php $this->render_icon_slot($fields[$field_key]) ?>
                </dt-location>
    ...
    public function post_form($post, $fields, $post_settings)
    {
        ...
        ?>
        <div class="grid-x" id="form-content">
            <input id="post_id" type="hidden" value="<?php echo esc_html(!empty($post) ? $post['ID'] : ''); ?>" />
            <input id="post_type" type="hidden" value="<?php echo esc_html(!empty($post) ? $post['post_type'] : ''); ?>" />
            <?php
            // Revert back to dt translations
            $this->hard_switch_to_default_dt_text_domain();
            ?>
